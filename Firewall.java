...
public class Firewall implements IFirewallService, IOFMessageListener,
    ...
    public void enableFirewall(boolean enabled) {
        logger.info("Setting firewall to java firewall {}", enabled);
        // mutation 1
    ...
    public boolean isEnabled(){
    	logger.info("Checking firewall status", enabled);
    	return this.enabled;
    }
    ...
    public List<FirewallRule> getRules() {
        // mutation 3
    	//return null;
    	logger.info(" checking number of firewall rules....." + this.rules.size() );
    	return this.rules;
    }
    ...
    public synchronized void addRule(FirewallRule rule) {
        ...
        logger.info(" adding a firewall rule with ID...." + rule.ruleid);
        
        int i = 0;
        // locate the position of the new rule in the sorted arraylist
        // mutation 2
        //for (i = 1; i < this.rules.size(); i++) {
...
/*
 // mutation 5
    ...
    @Override
  public synchronized void deleteRule(int ruleid) {
    	logger.info("deleting a firewall rule....");
        Iterator<FirewallRule> iter = this.rules.iterator();
        while (iter.hasNext()) {
            FirewallRule r = iter.next();
            // mutation 4
